 ...
 * 液滴的按压与拖拽驱动器。
 *
 * 五条独立弹簧驱动全部动效，各自拥有独立的阻尼与刚度：位置（临界，以 Tab 为单位
 * 而非像素，速度按 Tab 数归一化，不同屏幕密度手感一致）、速度（欠阻尼，归一化速度
 * 映射为拉伸形变）、按压进度（临界）、横向与纵向缩放（欠阻尼）。松手策略是「等液滴
 * 接近目标再回落缩放」，使甩动读起来是一个连贯动作。
 *
...
internal class DropletGestureDriver(
     ...
     * 整栏缩放的作用对象。
     *
     * 玻璃层、Tab 与液滴必须同步呼吸：仅放大玻璃层会让 Tab 明显脱队，因此缩放施加在宿主容器上。
     */
    ...
    /**
     * 宿主重建动态 Tab 行后重新绑定：取消进行中的手势，避免旧几何下的半程弹簧
     * 继续作用于新行；选中观察器会在新行布局完成后吸附到当前 Tab。
     */
    ...
    /**
     * 速度采样与归一化：按位置差分并除以 Tab 数范围，使拉伸形变与 Tab 宽度、屏幕密度无关。
     */
